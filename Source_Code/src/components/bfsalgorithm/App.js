...
if(detail[num]===1)
return <div id={num} className = "d-cell" style = {{background:"white"}} onClick={handleChange}></div>
else if(detail[num]===0)
return <div id={num} className = "d-cell" style = {{background: "black"}} onClick={handleChange}></div>;
else if(detail[num]===5)
return <div id={num} className = "d-cell" style = {{background: "red"}} onClick = {handleChange}></div>;
else if(detail[num]===6)
return <div id={num} className = "d-cell" style = {{background: "green"}} onClick = {handleChange}></div>;
}
...
return <div className = "d-row">{cells}</div>
  ...
  return (
    <div>
    <div className="algo-top"><span className="algo-title">{"Breadth First Search Algorithm"}</span>
<span>
<button value = "0" id="d-start" onClick = {handleClick} >Starting Point</button>
<button value = "1" id = "d-target" onClick = {handleClick}>Target</button>
<button value = "2"  id = "d-modify" onClick = {handleClick}>Modify Maze</button>
<Generate rows = {rows} id = "d-generate" columns = {columns} sendData = {getData}/>
</span>
</div>
