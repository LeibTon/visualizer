...
{
setDisabled(true)
setArray(sudokuProblems[Math.floor(Math.random()*sudokuProblems.length)])
}
  ...
  function inputField(index,p_index)
{
  ...
  return <input id = {9*p_index+index} className="sudo-input" type="text" style = {{color: "red"}} value = {array[0][9*p_index+index]} disabled = {isDisabled} onChange = {isDisabled ? null: handleChange}/>
  else
    return <input id = {9*p_index+index} className="sudo-input" type="text" value = "" disabled = {isDisabled} onChange = {isDisabled ? null: handleChange} />
}
...
return <div className = "singlerow">{input_Array}</div>
  ...
  return (
    <div>
    <div className="algo-top"><span className="algo-title">Backtracking</span>
<span>Sudoku Solver</span>
<span>
<button value="input" id="userbutton" onClick={handleClick}>Your Input</button>
<button value="random" id = "randombutton" onClick={handleClick}>Random</button>
</span>
</div>
<div className="bubble-main"> <Solve isDisabled = {isDisabled} array = {array} /> 
    <div className = "sudoku-master">
	  {container_array.map(singleRow)}
	</div>
    </div>
    </div>
  );
