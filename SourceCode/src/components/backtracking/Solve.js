...
if(grid[row][col] > 0)
return await solveSudoku(keys, grid, row, col+1)
...
keys[9*row+col].value = num;
await timeout(speed)
if(await solveSudoku(keys,grid,row,col+1))
return true;
}
grid[row][col] = 0;
await timeout(speed)
...
if(result)
setMessage(["Solved","green"])
else
setMessage(["Can't be solved","red"])
